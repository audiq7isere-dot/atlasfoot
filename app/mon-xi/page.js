...
<div className="xiGrid"><section><div className="pitch interactivePitch">{formations[formation].map((slot,i)=>{const usedElsewhere=new Set(selected.filter((name,index)=>index!==i&&name));return <div className="player pickPlayer" key={i} style={{left:slot.x+'%',top:slot.y+'%'}}><div className="shirt">{i+1}</div><select value={selected[i]||''} onChange={e=>choose(i,e.target.value)}><option value="">{slot.r}</option>{squad.filter(p=>eligible(slot.r,p)&&(!usedElsewhere.has(p.name)||selected[i]===p.name)).map(p=><option value={p.name} key={p.name}>{p.name}</option>)}</select></div>})}</div></section>
