...
export default function EngagementHub(){
 const [user,setUser]=useState(null),[votes,setVotes]=useState([]),[choice,setChoice]=useState(''),[preds,setPreds]=useState([]),[hs,setHs]=useState(1),[as,setAs]=useState(0),[msg,setMsg]=useState(''),[match,setMatch]=useState(null),[matchLoading,setMatchLoading]=useState(true)
 ...
 async function loadPredictions(matchKey){
  const {data:p}=await supabase.from('predictions').select('home_score,away_score,points').eq('match_key',matchKey)
  setPreds(p||[])
 }
 ...
 <div className="card engageCard"><span className="eyebrow">🏆 PRONOSTICS ATLASFOOT</span>{matchLoading?<><h2>Prochain match</h2><p>Chargement du prochain match du Maroc…</p></>:match?<><h2>{match.home} — {match.away}</h2><p><strong>{match.competition}</strong><br/>{formatDate(match.date)}{match.venue?' · '+match.venue:''}</p><p>Donne ton score avant le coup d’envoi. Les bons pronostics alimenteront le classement des supporters.</p><div className="scorePick"><input type="number" min="0" max="20" value={hs} onChange={e=>setHs(e.target.value)}/><strong>—</strong><input type="number" min="0" max="20" value={as} onChange={e=>setAs(e.target.value)}/></div><button className="btn red full" onClick={predict}>Valider mon pronostic</button><small>{preds.length} pronostic{preds.length>1?'s':''} enregistré{preds.length>1?'s':''}</small></>:<><h2>Prochain match du Maroc</h2><p>Le prochain match sera affiché ici dès qu’il sera disponible.</p></>}</div></div>
