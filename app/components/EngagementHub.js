...
export default function EngagementHub(){
 ...
 useEffect(()=>{
  ...
  const pollChannel=supabase.channel('home-poll-live').on('postgres_changes',{event:'*',schema:'public',table:'cafe_posts'},loadPoll).on('postgres_changes',{event:'*',schema:'public',table:'cafe_poll_votes'},loadPoll).subscribe()
  return()=>{clearInterval(clock);s.subscription.unsubscribe();supabase.removeChannel(pollChannel)}
 },[])

 async function loadPoll(){
  const {data:p,error}=await supabase.from('cafe_posts').select('id,content,poll_question,poll_options,created_at').eq('post_type','poll').eq('is_hidden',false).order('created_at',{ascending:false}).limit(1).maybeSingle()
  if(error||!p||!Array.isArray(p.poll_options)||p.poll_options.length<2){setPoll(null);setVotes([]);return}
  setPoll({id:p.id,question:p.poll_question||p.content||'Sondage AtlasFoot',choices:p.poll_options})
  const {data:v}=await supabase.from('cafe_poll_votes').select('user_id,option_index').eq('post_id',p.id)
  setVotes(v||[])
 }
 ...
 const counts=useMemo(()=>Object.fromEntries((poll?.choices||[]).map((_,i)=>[i,votes.filter(v=>v.option_index===i).length])),[poll,votes])
 ...
 async function vote(index){if(!poll)return;if(!user){setMsg('Connecte-toi au Café des Lions pour voter.');return}const {error}=await supabase.from('cafe_poll_votes').upsert({post_id:poll.id,user_id:user.id,option_index:index},{onConflict:'post_id,user_id'});if(error){setMsg(error.message);return}setMsg('Vote enregistré 🇲🇦');loadPoll()}
 ...
 return <section className="engagementHub"><div className="engageGrid"><div className="card engageCard"><span className="eyebrow">📊 SONDAGE DU JOUR</span>{poll?<><h2>{poll.question}</h2>{poll.choices.map((c,i)=>{const n=counts[i]||0,p=votes.length?Math.round(n*100/votes.length):0;return <button className="pollChoice" key={c+'-'+i} onClick={()=>vote(i)}><span>{c}</span><b>{p}%</b><i style={{width:p+'%'}}/></button>})}<small>{votes.length} vote{votes.length>1?'s':''} · résultats en direct</small></>:<><h2>Aucun sondage actif</h2><p>Le prochain sondage publié dans le Café des Lions apparaîtra automatiquement ici.</p><Link className="btn red full" href="/cafe">Créer ou voir les sondages</Link></>}</div>
