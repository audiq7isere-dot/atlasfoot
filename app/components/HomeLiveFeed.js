...
export default function HomeLiveFeed(){
 ...
 async function load(){try{const r=await fetch('/api/fil-actualite?ts='+Date.now(),{cache:'no-store'});const j=await r.json();setItems((j.items||[]).slice(0,16))}finally{setLoading(false)}}
 ...
 return <aside className="homeLiveRail">
   <div className="homeLiveHead"><div><span className="liveDot"></span><b>Fil d’actualité</b><small>Actualisé toutes les 2 minutes</small></div><Link href="/fil-actualite">Tout voir →</Link></div>
