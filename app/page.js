...
const videos=[
 ...
 {id:'itWn89uTqtU',cat:'MAROCAINS DU MONDE',title:'Youssef El Arabi avec l’Olympiakos'}
]
...
<section className="homeVideos"><div className="sectionTitle"><div><span className="eyebrow">🎥 ATLASFOOT TV</span><h2>Vidéos</h2></div><Link className="btn dark" href="/videos">Voir toutes les vidéos et interviews →</Link></div><div className="homeVideoGrid">{videos.map((v,i)=><a className={'videoCard videoLinkCard '+(i===0?'videoFeatured':'')} key={v.id} href={'https://www.youtube.com/watch?v='+v.id} target="_blank" rel="noreferrer"><div className="videoThumb"><img src={'https://i.ytimg.com/vi/'+v.id+'/hqdefault.jpg'} alt=""/><span className="playBadge">▶</span></div><div className="videoInfo"><span className="eyebrow">{v.cat}</span><h3>{v.title}</h3><small>Ouvrir sur YouTube</small></div></a>)}</div></section>
