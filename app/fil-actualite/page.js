...
const cats=['Tout','Lions de l’Atlas','Lionnes de l’Atlas','Botola Pro','Marocains du monde','Marocaines du monde','Mercato','Jeunes talents']
export default function FilActualite(){
 ...
 <div className="feedGrid"><section className="feedList">{loading&&<div className="card empty">Chargement des dernières informations…</div>}{!loading&&visible.length===0&&<div className="card empty">Aucune information disponible pour le moment.</div>}{visible.map((x,i)=><Link className="feedItem" key={x.link+i} href={'/actualite/'+x.slug}><div className="feedTime">{time(x.publishedAt)}</div><div className="feedBody"><div className="feedTop"><span className="feedCategory">{x.category}</span><span className="feedSource">{x.source}</span></div><h2>{x.title}</h2></div><div className="feedArrow">›</div></Link>)}</section>
 <aside className="feedAside"><div className="card"><span className="eyebrow">🇲🇦 ATLASFOOT</span><h3>Le fil 100% Maroc</h3><p>Le fil rassemble en continu les informations sur les Lions et Lionnes de l’Atlas, la Botola, les joueurs et joueuses marocains à l’étranger, le mercato et les jeunes talents.</p></div><div className="card"><h3>☕ Réagir avec les supporters</h3><p>Une actualité te fait réagir ? Viens en débattre dans le Café des Lions.</p><Link className="btn red full" href="/cafe">Ouvrir le Café</Link></div></aside></div></main></>}
