...
export default function FilActualite(){
 ...
 <div className="feedGrid"><section className="feedList">{loading&&<div className="card empty">Chargement des dernières informations…</div>}{!loading&&visible.length===0&&<div className="card empty">Aucune information disponible pour le moment.</div>}{visible.map((x,i)=><Link className="feedItem" key={x.link+i} href={articleHref(x)}><div className="feedTime">{time(x.publishedAt)}</div><div className="feedBody"><div className="feedTop"><span className="feedCategory">{x.category}</span><span className="feedSource">{x.source}</span></div><h2>{x.title}</h2></div><div className="feedArrow">›</div></Link>)}</section>
